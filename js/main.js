...
// make hwc functions merged with jquery
// for a more friendly suntax
jQuery.extend( $ , hwc );
...
function listener() {
    ...
    $('#incidence_matrix').bind('input propertychange', function () {
        graphAlgs.updateByTextArea();
    });

    $("#selectAlgorithm").change(function () {
        var algorithm = $('#selectAlgorithm').val();
        graphAlgs.alg=new graphAlgs[algorithm](graphAlgs);
        graphAlgs.renderGraphViz(graphAlgs.nodeData);
    });
}
...
var graphAlgs = {
    ...
    renderGraphViz: function (nodeData) {
        ...
        for (var e in nodeData.edges) {
            for (var node in nodeData.edges[e]) {
                if (!that.alg.weighted) {
                    nodeData.edges[e][node]._cost=nodeData.edges[e][node].cost;
                    nodeData.edges[e][node].cost = null;
                } else {
                    if (nodeData.edges[e][node].cost === null && nodeData.edges[e][node]._cost)
                        nodeData.edges[e][node].cost=nodeData.edges[e][node]._cost
                }

                nodeData.edges[e][node].directed=that.alg.directed;
            }
        }
        ...
        if (this.graph) {
            this.graph.eachNode(
                function (node, pt) {
                    that.graph.pruneNode(node);
                }
            );

            this.graph.graft(nodeData);
        } else {
            var graph = arbor.ParticleSystem(1000, 600, 0.5) // create the graphtem with sensible repulsion/stiffness/friction
    ...
    runAlgorithm: function (algorithm) {
        ...
        this.alg=new this[algorithm](this);

        this.stepByStep ? this.alg.step_by_step() : this.alg.process();
        ...
        $('.graphType').click(
            function () {
                ...
                switch (graphSelected) {
                    case 'simpleGraph':
                        thisObj.renderGraphViz(thisObj.getSimpleGraph());
                        break;

                    case 'graph1':
                        thisObj.renderGraphViz(thisObj.getGraph1());
                        break;

                    case 'graph2':
                        thisObj.renderGraphViz(thisObj.getGraph2());
                        break;

                    case 'graph3':
                        thisObj.renderGraphViz(thisObj.getGraph3());
                        break;
    ...
    init: function () {
        ...
        var algorithm = $('#selectAlgorithm').val();
        this.alg=new graphAlgs[algorithm](this);

        this.updateByTextArea();
    ...
    updateByTextArea: function () {
        var that = this;
        var el = document.getElementById('incidence_matrix');
        el && eval(el.value);
        ...
        // matrix dimension
        var m = Matrix_Incidence.length;
        var n = Matrix_Incidence[0].length;

        // create labels
        for (i = 0; i < n; i++) {
            // if no label defined, we use the index
            var label = i;
            if (Matrix_Labels && Matrix_Labels[i])
                label = Matrix_Labels[i];

            matrix.nodes[i] = { 'color': 'black', 'shape': 'dot', 'label': label };
        }

        $(Matrix_Incidence).each(function (i, e) {
            var edge = {};

            $(e).each(function (i, e) {
                if (e) {
                    edge[i] = {
                        directed: that.alg.directed
                    };
